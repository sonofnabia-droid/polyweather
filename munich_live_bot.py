...
def display(now, latest_obs, temps_by_hour, series_today, signals, p,
    ...
    if not stop_loss_hit:
        if bet_placed:
            prompt_txt = (f"  {C['yellow']}{B}◈  Já apostaste hoje. "
                          f"Forçar nova entrada? → escreve {C['white']}f{C['yellow']} + Enter{R}")
        else:
            prompt_txt = (f"  {C['yellow']}{B}◈  Entrar no mercado na mesma? "
                          f"→ escreve {C['white']}f{C['yellow']} + Enter{R}")
...
def run(wu_key: str, threshold: float, bankroll: float,
    ...
    try:
        while True:
            ...
            # - Se o mercado é de outro dia (ex: amanhã) → previsão WU
            #   Fallback para rmax se a previsão não estiver disponível.
