...
optimizer.py
============
Otimização de thresholds - test sistemático de diferentes combinações.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Optimizer de Thresholds")
    ...
    print(f"\n  Optimizer: {start_date} → {end_date}  modo={mode}")
    ...
    if mode in ("both", "single"):
        print("\n  === MODO SINGLE ===")
        single_thresholds = [0.65, 0.70, 0.75, 0.80, 0.85]

        for thr in single_thresholds:
            print(f"    threshold={thr:.0%}...", end=" ", flush=True)
    ...
    if mode in ("both", "phased"):
        print("\n  === MODO PHASED ===")
        p1_mins = [0.20, 0.30, 0.40]
        p1_maxes = [0.60, 0.65, 0.70]
        p2_thrs = [0.65, 0.70, 0.75, 0.80]
        p3_thrs = [0.80, 0.85, 0.90]

        print(f"    Testing {len(p1_mins)*len(p1_maxes)*len(p2_thrs)*len(p3_thrs)} "
              f"combinations...")
    ...
    out_path = OUTPUT_DIR / "optimizer_results.csv"
