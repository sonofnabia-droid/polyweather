"""
munich_model.py — UNIFIED
===============
Carregamento de modelos (LightGBM + XGBoost), construção de features
V1+V2, predição ensemble com predict_proba(), z-score streaming,
gestão do histórico diário de máximas.

Exporta:
  load_models()                                         — dict com modelos + config
  load_model()                                          — retrocompat (tuple)
  build_features(slots_so_far, current, month, doy)
  predict_p(model, feat_cols, slots_so_far, current, month, doy)  — LGBM only
  predict_ensemble(models, slots_so_far, current, month, doy, zscore)  — ensemble
  StreamingPeakDetector                                 — z-score detector
...
NOTAS:
  - predict_ensemble() NÃO recebe feat_cols — vem de models["feat_cols"].
  - Usa predict_proba() NÃO predict() — probabilidades calibradas [0,1].
  - Features V2 (dewpoint, pressure, wind, foehn) são construídas com
    defaults se os dados WU não as fornecerem.
"""
...
def load_models() -> dict:
    """
    Carrega modelos LightGBM + XGBoost e config associada.
    Devolve dict com: model_lgb, model_xgb, feat_cols, prior_map,
    doy_poly, ensemble_weights.
    """
    ...
    # DOY polynomial threshold curve
    ...
    # Ensemble weights (com redistribuição se XGB ausente)
...
def load_model():
    """
    Retrocompatibilidade — retorna (model_lgb, feat_cols, prior_map).
    """
    result = load_models()
    return (
        result["model_lgb"],
...
# ══════════════════════════════════════════════════════
#  FEATURE BUILDER — V1 (18) + V2 (7) = 25 FEATURES
# ══════════════════════════════════════════════════════
# V1 (18 canónicas): usadas pelo modelo treinado (FEATURE_COLS)
# V2 (7 extra): construídas com defaults, disponíveis se o modelo as usar
# A filtragem final é feita em predict: avail = [f for f in feat_cols if f in row]

def build_features(slots_so_far: list[dict], current: dict,
                   month: int, doy: int, minute: int = 0) -> dict:
    """
    Constrói features V1+V2 para o slot actual.
    slots_so_far: lista cronológica incluindo o slot actual como último.
    """
    ...
    # Wind south proxy: alinhamento com 180° (Sul — Föhn)
    if 135 <= wdir <= 225:
    ...
    return {
        # ── V1 (18 canónicas — alinhadas com FEATURE_COLS) ──
        "slot_frac":           slot_frac,
        ...
        # ── V2 (7 extra — disponíveis se o modelo as usar) ──
        "dewpoint_c":           dewpt,
...
def predict_p(model, feat_cols, slots_so_far: list[dict], current: dict,
    ...
    Usa predict_proba() para probabilidades calibradas [0,1].
    """
    hour = current["hour"]
    ...
    # predict_proba()[:, 1] = probabilidade calibrada da classe positiva
...
def predict_ensemble(
    ...
    Retorna dict com:
      p_ensemble, p_lgbm, p_xgb, p_zscore, weights, components
    """
    hour = current["hour"]
    if len(slots_so_far) < 4 or hour < MIN_HOUR:
        return {
...
class StreamingPeakDetector:
    ...
    Sinal composto:
      40% — z-score (quão acima da média está a temp actual)
      35% — slope (se a temperatura está a descer = sinal de pico)
      25% — pct_of_max (proximidade do running max)
    """
    ...
    def update(self, temp: float) -> float:
        """Actualiza o detector com nova temperatura. Retorna P(pico)."""
        ...
        # Slope signal (negativo = pico passado)
        if n >= 4:
...
def init_history_max() -> dict:
    """Carrega histórico de máximas de ficheiro."""
...
def save_history_max(history_max: dict) -> None:
    """Guarda histórico de máximas em ficheiro."""
    path = Path("live_history_max.json")
...
def update_history_max(history_max: dict, slots_so_far: list[dict]) -> None:
    """Actualiza máxima de hoje no histórico."""
    if not slots_so_far:
        return
    today = berlin_date()
